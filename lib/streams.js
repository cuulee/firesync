...
FirebaseChildStream.prototype._read = function() {
  ...
  function updateChild(child) {
    stream.push({ op: 'update', id: child.key(), child: child });
  }

  function removeChild(child) {
    stream.push({ op: 'remove', id: child.key(), child: child });
  }

  this.ref.on('child_removed', removeChild);
  this.ref.on('child_changed', updateChild);
  this.ref.on('child_added', updateChild);
};
...
function ElasticBulkStream(client, opts) {
  ...
  this.type = opts.type;
  this.map = opts.map || function(child) { return child.val(); };
  this.body = [];
}
...
function _addToBulkBody(chunk) {
  var idDoc = { _id: chunk.id };

  switch(chunk.op) {
    case 'index':
      this.body.push(
        { index: idDoc },
        { doc: this.map(chunk.child) }
      );
      break;
    case 'update':
      this.body.push(
      ...
      break;
    case 'remove':
      this.body.push({ delete: idDoc });
      break;
    default:
      throw new Error('invalid operation ' + chunk.op);
  }
}
...
function LogStream(logfn) {
  ...
  this.log = logfn || function(chunk) {
    debug('[%s] key: %s', chunk.op, chunk.id);
  };
}
